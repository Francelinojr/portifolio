...
const Navbar = memo(function Navbar() {
  ...
  // Derive active item from location and hash without setting state in effects
  let active = '#home';
  if (location.pathname.includes('/certificates')) {
    ...
    active = '/projects';
  } else if (location.hash || currentHash) {
    active = location.hash || currentHash;
  }

  const linkClass = (target: string) =>
    `px-4 py-1.5 rounded-full transition-all duration-300 text-sm font-medium ${active === target
